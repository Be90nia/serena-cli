...
LIB="$PROJ/lib.rs"
MAIN="$PROJ/main.rs"
LOG=local/acceptance/a-rust.log.md
...
t() { # t <name> <args...>
  ...
  local t0 t1 rc out
  t0=$(date +%s%3N)
  out=$("$B" --direct --project "$PROJ" --json "$@" 2>&1)
  rc=$?
  t1=$(date +%s%3N)
  {
    echo "### $name | rc=$rc | ms=$((t1 - t0))"
    echo '```'
    echo "$out"
    echo '```'
    echo
...
ITEM=$("$B" --direct --project "$PROJ" --json call-hierarchy prepare "$MAIN" 3 12 2>/dev/null | jq -c '.[0]' 2>/dev/null)
