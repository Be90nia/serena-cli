...
use lsp_core::workspace_folders::{discover_additional_workspace_folders, primary_workspace_folder};
...
async fn monorepo_workspace_folders_reach_mock_ls_via_session() {
    ...
    for _ in 0..50 {
        if log_path.is_file() && std::fs::metadata(&log_path).map(|m| m.len() > 0).unwrap_or(false)
        {
            break;
        }
    ...
    let names: std::collections::BTreeSet<&str> =
        wf.iter().filter_map(|f| f.get("name").and_then(|n| n.as_str())).collect();
